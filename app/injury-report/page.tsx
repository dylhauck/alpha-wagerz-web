...
function getInjuryLabel(player: InjuryPlayer) {
  ...
  for (let index = sentences.length - 1; index >= 0; index -= 1) {
    ...
    if (injuryKeywords.some((keyword) => lowered.includes(keyword))) {
      return sentence;
    }
  }
...
export default function InjuryReportPage() {
  ...
  useEffect(() => {
    fetch("/data/injury_report.json", { cache: "no-store" })
      .then((response) => {
        if (!response.ok) {
          throw new Error(
  ...
  const players = useMemo(() => {
    ...
    return (report?.players ?? [])
      .filter((player) => {
        const matchesStatus =
          status === "ALL" || player.availability === status;

        const injuryLabel = getInjuryLabel(player).toLowerCase();

        const matchesSearch =
          !search ||
          player.player.toLowerCase().includes(search) ||
          player.team.toLowerCase().includes(search) ||
          injuryLabel.includes(search) ||
      ...
      .sort((a, b) => {
        const statusOrder =
          Number(a.availability === "DTD") -
          Number(b.availability === "DTD");

        if (statusOrder !== 0) {
          return statusOrder;
        }

        const teamOrder = a.team.localeCompare(b.team);
  ...
  return (
    <AppShell>
      <div className="space-y-5 py-5">
        <section className="overflow-hidden rounded-[28px] border border-cyan-300/15 bg-slate-950/70 px-5 pb-5 pt-2 shadow-2xl shadow-black/20">
          <div className="flex w-full flex-col items-center text-center">
            ...
            <div className="-mt-3 text-sm font-black uppercase tracking-[0.34em] text-slate-400">
              {today}
            </div>
        ...
        <section className="overflow-hidden rounded-2xl border border-white/10 bg-slate-950/60">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1120px]">
              <thead className="bg-white/[0.025] text-left text-[11px] font-black uppercase tracking-[0.16em] text-slate-500">
                <tr>
                  <th className="px-5 py-4">Player</th>
                  <th className="px-4 py-4">Team</th>
                  <th className="px-4 py-4">Pos</th>
                  <th className="px-4 py-4">Status</th>
                  <th className="px-4 py-4">Injury / Reason</th>
                  <th className="px-4 py-4">Estimated Return</th>
              ...
              <tbody>
                {players.length === 0 ? (
                  <tr>
                    ...
                    >
                      No matching injury records were found.
                    </td>
                  </tr>
                ) : (
                  players.map((player) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-4 text-sm font-bold text-cyan-200">
                        {player.estimated_return || "Unknown"}
                      </td>

                      <td className="px-4 py-4">
                        <div className="text-sm font-bold text-slate-300">
                          {player.espn_status || player.availability}
                        </div>
