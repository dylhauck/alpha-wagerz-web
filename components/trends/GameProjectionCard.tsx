    ...
    <div className="rounded-2xl border border-white/10 bg-[#11182c] p-4">
      ...
      <div className="mt-3 grid grid-cols-3 gap-2 text-center">
        ...
        <div>
          ...
          <div className="mt-1 text-sm font-black text-pink-200">
            {edge}
          </div>
        </div>
      </div>
...
export function GameProjectionCard({
  projection,
}: {
  ...
  function openAwayPitcherProfile() {
    setSelectedPlayer({
      playerId:
        projection.away_pitcher_id ||
        projection.awayPitcherId ||
        projection.away_pitcher_mlb_id,

      playerName: projection.away_pitcher || "",
      teamName: projection.away_team || "",

      teamId:
        projection.away_team_id ||
  ...
  function openHomePitcherProfile() {
    setSelectedPlayer({
      playerId:
        projection.home_pitcher_id ||
        projection.homePitcherId ||
        projection.home_pitcher_mlb_id,

      playerName: projection.home_pitcher || "",
      teamName: projection.home_team || "",

      teamId:
        projection.home_team_id ||
      ...
      <div className="rounded-3xl border border-cyan-300/15 bg-white/[0.035] p-5">
        <div className="mb-4 flex items-start justify-between gap-4">
          <div>
            ...
            <div className="text-sm font-semibold text-slate-400">
              {projection.venue} ·{" "}
              {projection.game_time || "Time TBD"}
            </div>
          </div>

          <EdgePill
            label={
              projection.best_lean ||
              projection.moneyline_lean
            }
          />
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-2xl border border-pink-300/15 bg-pink-300/10 p-4">
            <div className="text-xs font-black uppercase text-pink-100">
              Win Probability
            </div>
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
            ...
            <div className="mt-1 text-xs font-black uppercase tracking-[0.14em] text-cyan-200">
              {projection.total_recommendation ||
                projection.total_lean}
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 grid gap-3 xl:grid-cols-2">
          <MarketBox
            title="Moneyline"
            market={`${away} ${marketText(
              projection.market?.moneyline?.away,
            )} / ${home} ${marketText(
              projection.market?.moneyline?.home,
            )}`}
            model={`${projection.moneyline_lean} ${fmt(
              projection.moneyline_edge,
              "% edge",
            )}`}
            edge={fmt(projection.moneyline_edge, "%")}
            lean={
              projection.moneyline_recommendation ||
              projection.moneyline_lean
            }
          />
          ...
          <MarketBox
            title="Run Line"
            market={`${away} ${fmt(
              projection.market?.spread?.away,
            )} / ${home} ${fmt(
              projection.market?.spread?.home,
            )}`}
            model={`Projected margin ${fmt(
              projection.projected_margin,
            )}`}
            edge={fmt(projection.spread_edge)}
            lean={
              projection.spread_recommendation ||
              projection.spread_lean
            }
          />
          ...
          <MarketBox
            title="Team Totals"
            market={`${away} ${fmt(
              projection.market?.team_totals?.away,
            )} / ${home} ${fmt(
              projection.market?.team_totals?.home,
            )}`}
            model={`${away} ${fmt(
              projection.away_projected_runs,
            )} / ${home} ${fmt(
              projection.home_projected_runs,
            )}`}
            edge={`${away} ${fmt(
              projection.away_team_total_edge,
            )} / ${home} ${fmt(
              projection.home_team_total_edge,
            )}`}
            lean={
              projection.team_total_recommendation ||
              "No Team Total Lean"
            }
          />
        </div>
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            <div className="rounded-xl border border-cyan-300/10 bg-cyan-300/10 p-3">
              <div className="flex items-center gap-2">
                <PlayerNameButton
                  name={
                    projection.away_pitcher || "TBD"
                  }
                  onClick={openAwayPitcherProfile}
                  className="text-sm font-black"
                />

                <ThrowHandBadge
                  throws={
                    projection.away_pitcher_throws
                  }
                />
              </div>
              ...
              <div className="text-xs text-slate-400">
                Market:{" "}
                {fmt(
                  projection.away_pitcher_k_line,
                )}{" "}
                · Model:{" "}
                {fmt(
                  projection.away_projected_ks,
                )}{" "}
                K
              </div>

              <div className="mt-1 text-xs font-black uppercase text-cyan-200">
                {projection.away_pitcher_k_recommendation ||
                  "No K Lean"}
              </div>
            </div>
            ...
            <div className="rounded-xl border border-pink-300/10 bg-pink-300/10 p-3">
              <div className="flex items-center gap-2">
                <PlayerNameButton
                  name={
                    projection.home_pitcher || "TBD"
                  }
                  onClick={openHomePitcherProfile}
                  className="text-sm font-black"
                />

                <ThrowHandBadge
                  throws={
                    projection.home_pitcher_throws
                  }
                />
              </div>
              ...
              <div className="text-xs text-slate-400">
                Market:{" "}
                {fmt(
                  projection.home_pitcher_k_line,
                )}{" "}
                · Model:{" "}
                {fmt(
                  projection.home_projected_ks,
                )}{" "}
                K
              </div>

              <div className="mt-1 text-xs font-black uppercase text-pink-200">
                {projection.home_pitcher_k_recommendation ||
                  "No K Lean"}
              </div>
            </div>
          </div>
